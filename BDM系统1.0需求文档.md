...
| 本月底薪 | 根据本月底薪有效薪资流水匹配当前等级后，取该等级对应的现金底薪配置值；不含提成、奖励、扣款 |
| 新增公会未达标扣款 | 若本月新增有效公会数 < 配置值，则按配置比例 × (底薪 + 公会提成) |
...
- 政策从已有政策列表下拉选择，创建时绑定
- 月度新增有效公会目标及扣款比例仅BD角色显示
- 基础信息（BD ID、姓名）创建后不可更改；试用期截止日期可维护，但仅允许选择每月1号
...
| 本月新增公会数量 | 统计本月绑定到当前BD的公会 | 统计周期内只要公会绑定到当前BD即计入本月新增公会数量；不要求公会在本月创建，也不要求当前BD为首个绑定BD；同一公会在同一统计周期内对同一BD仅计一次 |
| 本月公会提成 | Bonus有效薪资流水 × 等级对应提成比例 | 根据Bonus有效薪资流水匹配等级配置表，获取该等级提成比例后计算 |
| 本月底薪 | 根据本月底薪有效薪资流水匹配当前等级后，取该等级对应的现金底薪配置值 | 根据底薪有效薪资流水匹配等级配置表，取该等级现金底薪值；该字段仅表示底薪本身，不含提成、奖励、扣款 |
| 本月总薪资 | 本月底薪 + 本月公会提成 | 不含预支、借支、扣款，仅底薪+提成 |
| 本月总薪资余额 | 本月总薪资 - 新增公会未达标扣款 | 实际可结算金额，红色标注（若为扣款状态） |
| 新增公会未达标扣款 | 若新增有效公会数 < 配置目标，则：(底薪+提成) × 扣款比例 | 查询BD绑定政策配置中的新增有效公会目标；若本月新增有效公会数低于目标数，触发扣款 |
...
- 新公会考核：每月至少配置目标数个新增有效公会，未完成按可配置百分比扣除（范围：底薪+提成奖励）。
- 新增公会未达标扣款判断口径：统一使用“本月新增有效公会数量”，不再使用“本月新增公会数量”。“本月新增公会数量”仅用于展示新增规模，是否达标以新增公会在Bonus有效区间内达到全局门槛后的“新增有效公会”结果为准。
- 新增公会口径（版本A）：只要公会在当前统计周期内绑定到当前BD，就可以计入“新增公会”；不要求公会在当月创建，也不要求当前BD为首个绑定BD。新增有效公会则是在上述新增公会基础上，进一步满足Bonus有效薪资流水达到全局门槛。

<span style="color: red;">**注意：** 公会有效薪资门槛金额、新增有效公会目标、扣款比例、等级配置、底薪有效时长、Bonus有效时长等参数均由服务端配置返回，禁止客户端硬编码。</span>
...
| BDM本月底薪 | 根据BDM团队底薪有效薪资流水匹配当前等级后，取该等级对应的现金底薪配置值；不含团队提成、奖励 | 根据BDM团队底薪有效薪资流水匹配BDM等级配置表，取该等级现金底薪值 |
...
- 换绑到新BD后，若发生在当前统计周期内，则可计入新BD的“本月新增公会数量”与“本月新增有效公会数量”；新增口径不再要求公会首次创建，也不再要求当前BD为首个绑定BD。
...
| 入驻满1个月 | 试用期考核判定；BD身份读取BD试用期转正门槛金额，BDM身份读取BDM试用期转正门槛金额；薪资达到对应门槛时可转正，未达标则状态仍为试用中 |

<span style="color: red;">**注意：** 上述数值（公会有效薪资门槛金额、法币提现开放期、BD试用期转正门槛金额、BDM试用期转正门槛金额）均由服务端配置返回，禁止客户端硬编码。</span>
...
| 本月新增公会数量 | 整数 | 统计本月绑定到当前BD的公会数量 | 公会服务：筛选统计周期内绑定BD为当前BD的公会；不要求公会在本月创建，也不要求当前BD为首个绑定BD；同一公会在同一统计周期内对同一BD仅计一次；该字段仅用于展示新增公会规模，不直接用于新增公会未达标扣款判断 |
...
| 本月底薪 | $格式 | 根据本月底薪有效薪资流水匹配当前等级后，读取该等级对应的现金底薪配置值；不含提成、奖励、扣款 | 薪资服务：根据底薪有效薪资流水匹配等级配置表，取对应底薪金额 |
| 新增公会未达标扣款 | -$格式 | 计算公式：若新增有效公会数<目标，则(底薪+提成)×扣款比例 | 薪资服务：查配置的新增有效公会目标→判断是否达标→若不达标按配置扣款比例计算 |
...
| 本月底薪 | $格式 | 根据BDM团队底薪有效薪资流水匹配当前等级后，取该等级对应的现金底薪配置值；不含团队提成、奖励 | 薪资服务：根据团队底薪有效薪资流水匹配BDM等级配置表，取对应底薪金额 |
...
展示BDM团队公会列表，支持按公会ID或公会名称搜索；团队公会卡片展示所属BD，便于BDM识别公会归属。
...
| 达标状态 | 已达标/未达标 | 按当前自然月 ∩ 绑定关系有效期 ∩ Bonus有效区间内Bonus有效薪资流水是否达到全局门槛判断；达到则展示“已达标”标签 | 薪资流水表 + 公会绑定关系表，按Bonus有效薪资流水口径计算 |
...
**公会明细搜索：**
1. 搜索框提示文案："输入ID或名称搜索公会"
2. 输入公会ID时，按公会ID精确匹配；输入公会名称时，按公会名称模糊匹配
3. 搜索范围仅限当前BDM团队公会范围：BDM管理范围内BD团队绑定的公会 + BDM本人作为BD绑定的个人公会
4. 页面不提供“所属BD”下拉筛选；所属BD仅作为团队公会卡片字段展示
5. 清空搜索框后，恢复展示全部团队公会

**公会列表：**
1. 列表展示所有团队公会，按入驻时间降序排列（最新入驻的在前）
2. 每张卡片包含公会名称、公会ID、达标状态、bonus到期时间、入驻时间、所属BD、主播人数、本周期薪资、本月薪资
3. 所属BD字段展示格式：BD昵称（ID：BD编码）
...
| 本月有效公会数量 | 整数 | 统计当前自然月∩绑定关系有效期∩Bonus有效区间内Bonus有效薪资流水达到全局门槛的公会 | 公会绑定关系表 + 薪资流水表，按Bonus有效薪资流水窗口计算每个公会薪资，达到全局门槛才计入 |
| 本月新增公会数量 | 整数 | 统计本月首次创建且首次绑定成功的公会 | 公会表+首次绑定关系表，筛选首次创建且首次绑定BD为当前BD且时间在本月内的公会 |
| 本月新增有效公会数量 | 整数 | 统计本月绑定到当前BD、且Bonus有效薪资流水达到全局门槛的公会 | 公会表+公会绑定关系表，筛选统计周期内绑定BD为当前BD且Bonus有效薪资流水达到全局门槛的公会；不要求公会在本月创建，也不要求当前BD为首个绑定BD；同一公会在同一统计周期内对同一BD仅计一次；BD新增公会未达标扣款统一以该字段作为判断口径 |
...
**搜索：**
1. 输入框提示文案："输入ID或名称搜索BD成员"
2. 输入BD ID时按BD ID精确匹配；输入BD名称时按BD名称模糊匹配
3. 搜索范围仅限当前BDM直属BD成员
4. 清空搜索框后，恢复展示全部团队成员
...
**页面加载：**
1. 接收BD ID参数
2. 顶部灰色卡片仅展示BD概要信息：BD名称、BD ID；不展示“所属公会总薪资”字段
3. 下方列表展示该BD绑定公会明细：公会名称、公会ID、bonus到期时间、入驻时间、主播人数、本周期薪资、本月薪资
...
| 公会加入时间 | 公会创建时间 | 直接读取公会创建时间 | 公会表，格式YYYY-MM-DD HH:mm:ss |
...
新增BD人员账号，配置基础信息、权限和政策等级。试用期信息改为配置“试用期截止日期”，不再录入试用期天数。
...
| 试用期截止日期 | 试用期结束日期 | 日期选择器写入试用期截止日期 | 用户选择，仅可选择每月1号 |
...
| 月度新增有效公会目标 | BD角色专属，每月新增有效公会数目目标 | 直接输入写入新增有效公会目标；新增有效公会定义为统计周期内绑定到当前BD且Bonus有效薪资流水达到全局门槛的公会，不要求公会在当月创建，也不要求当前BD为首个绑定BD | 用户输入，仅角色（role）=0时显示 |
| 月度新增有效公会目标未达标扣除工资比例 | BD角色专属，未达标扣款比例 | 直接输入写入扣款比例 | 用户输入，百分比格式，仅角色（role）=0时显示 |
...
4. 校验政策存在且状态为可绑定；BD政策必须为政策对象=BD，BDM政策必须为政策对象=BDM；职位为BD时校验BD政策必填并清空BDM政策；职位为BDM时校验BD政策和BDM政策均必填；试用期截止日期仅允许选择每月1号；停用政策时必须先校验是否存在在职BD正在使用该政策，若存在则禁止停用并提示“该政策已有在职BD使用中，无法停用。”
...
编辑BD人员信息、权限和政策配置。试用期信息改为维护“试用期截止日期”，不再录入试用期天数。
...
| 试用期截止日期 | 试用期结束日期 | 日期选择器写入试用期截止日期（预填当前值） | 用户选择，预填值来自试用期截止日期，仅可选择每月1号 |
...
| 月度新增有效公会目标 | BD角色专属，每月新增有效公会数目目标 | 直接输入写入新增有效公会目标（预填当前值） | 用户输入，预填值来自新增有效公会目标，仅角色（role）=0时显示 |
| 月度新增有效公会目标未达标扣除工资比例 | BD角色专属，未达标扣款比例 | 直接输入写入扣款比例（预填当前值） | 用户输入，预填值来自扣款比例，百分比格式，仅角色（role）=0时显示 |
...
**表单修改：**
1. 可修改姓名、职位、在职状态、归属TEAM、试用期截止日期、审核公会申请权限、金币提现权限、推荐人、BD政策、BDM政策、月度新增有效公会目标、未达标扣款比例等人员字段
2. BD ID灰色显示，不可编辑
...
配置BD/BDM两种角色的薪资政策，包括政策基础信息、等级配置、奖励、提成比例、底薪、月度新增有效公会目标及未达标扣款比例。页面支持按政策对象、政策名称、状态筛选，支持新建政策和编辑已有政策。
...
| 月度新增有效公会目标 | BD角色专属，每月新增有效公会数目目标（仅BD政策有此字段） | 直接读取新增有效公会目标；新增有效公会定义为统计周期内绑定到当前BD且Bonus有效薪资流水达到全局门槛的公会，不要求公会在当月创建，也不要求当前BD为首个绑定BD | 政策配置表或等级配置扩展字段，仅政策对象（policy_target）=BD时生效 |
| 月度新增有效公会目标未达标扣除工资比例 | BD角色专属，未达标时扣除工资比例（仅BD政策有此字段） | 直接读取扣款比例 筛选政策对象为BD（政策对象（policy_target）=BD） | 政策配置表或等级配置扩展字段，百分比格式，仅政策对象（policy_target）=BD时生效 |
...
- 停用政策时，先检测该政策是否仍被在职BD使用；若存在任一在职BD正在使用该政策，则拦截停用操作并弹窗提示："该政策已有在职BD使用中，无法停用。"
- 等级配置按有效薪资目标升序排序
...
配置BD政策全局门槛，影响所有BD/BDM薪资有效流水判定和试用期转正规则。试用期转正门槛按BD和BDM分别配置，BD转正读取BD试用期转正门槛金额，BDM转正读取BDM试用期转正门槛金额。
...
| BD试用期转正门槛金额$（≥） | 可定义BD试用期内绑定公会薪资大于等于该值时，可自动进行转正 | 直接输入金额，仅可输入大于等于0的数字，默认1000$ | 用户输入，保存到BD政策全局配置 |
| BDM试用期转正门槛金额$（≥） | 可定义BDM试用期内团队薪资大于等于该值时，可自动进行转正 | 直接输入金额，仅可输入大于等于0的数字，默认1000$ | 用户输入，保存到BD政策全局配置 |
...
| 基础信息区 | 政策名称、政策对象、是否启用、月度新增有效公会目标、月度新增有效公会目标未达标扣除工资比例、底薪有效时长、Bonus有效时长 | 按原型顺序上下排列 |
...
| 月度新增有效公会目标 | BD角色专属，每月需达成的新增有效公会数目目标 | 直接输入整数；新增有效公会定义为统计周期内绑定到当前BD且Bonus有效薪资流水达到全局门槛的公会，不要求公会在当月创建，也不要求当前BD为首个绑定BD | 用户输入；政策对象为BD时必填，BDM时不可编辑或隐藏 |
| 月度新增有效公会目标未达标扣除工资比例 | BD角色专属，未达标时扣除工资的百分比 | 直接输入百分比 | 用户输入；政策对象为BD时必填，BDM时不可编辑或隐藏 |
...
- BD政策必须配置月度新增有效公会目标和未达标扣款比例
- BDM政策不参与BD新增公会未达标扣款，月度新增有效公会目标和扣款比例不可编辑或隐藏
...
2. 政策对象：必选
3. 月度新增有效公会目标：政策对象为BD时必填，BDM时不可编辑或隐藏
...
**预填规则：**
- 基础信息区：显示当前政策名称、政策对象、新增有效公会目标、扣款比例
- 等级配置表：显示当前政策的所有等级配置行

#### 交互逻辑

**表单修改：**
1. 可修改政策名称、新增有效公会目标、扣款比例、底薪有效时长、Bonus有效时长、等级配置表所有字段
...
平台需要管理BD与公会的绑定关系，支持查看所有绑定记录、新增绑定、编辑绑定（换绑）等操作。该模块与"公会管理模块"的区别在于：公会管理侧重公会基础信息的CRUD，而本模块侧重BD归属关系的维护。

### 对应原型

![后台-BD绑定公会详情](1.0版本原型图/后台/后台-bd绑定公会详情.png)

![后台-BD绑定公会详情-新建/编辑](1.0版本原型图/后台/后台-bd绑定公会详情-新增_编辑.jpg)
...
展示平台所有BD与公会的绑定关系，支持多维度筛选查询、查看详情、新增绑定、编辑绑定等操作。
...
登录后台后，从左侧导航进入"BD管理" -> "BD绑定公会详情"页面。
...
| 字段 | 说明 | 控件类型 | 数据来源 |
|---|---|---|---|
| 所属BD | 输入BD ID查询 | 输入框 | 用户输入 |
| 所属BDM | 输入BDM ID查询 | 输入框 | 用户输入 |
| 公会ID | 输入公会ID查询 | 输入框 | 用户输入 |
| 公会长ID | 输入公会长ID查询 | 输入框 | 用户输入 |
| bonus到期日期 | 日期范围选择 | 日期选择器 | 用户选择 |
| bonus到期状态 | 已到期/未到期/全部 | 下拉选择 | 前端枚举 |
| 大区 | 下拉选择大区 | 下拉选择 | 公会系统 |
...
| 字段 | 说明 | 统计方式 | 数据来源 |
|---|---|---|---|
| 当月Bonus未到期公会数量 | 筛选范围内Bonus未到期公会总数 | 统计(公会ID) 筛选 Bonus到期时间 > 当前时间 且筛选条件 | 公会绑定关系表 |
| 当月底薪有效薪资流水 | 筛选范围内所有公会按底薪有效期计入的有效薪资流水汇总 | 对每个公会取当前自然月∩绑定关系有效期∩底薪有效区间内有效薪资流水，窗口薪资达到全局门槛则全额计入，低于门槛则0计入 | 薪资流水表 + 公会绑定关系表，按底薪有效薪资流水口径汇总 |
| 当月Bonus有效薪资流水 | 筛选范围内所有公会按Bonus有效期计入的有效薪资流水汇总 | 对每个公会取当前自然月∩绑定关系有效期∩Bonus有效区间内有效薪资流水，窗口薪资达到全局门槛则全额计入，低于门槛则0计入 | 薪资流水表 + 公会绑定关系表，按Bonus有效薪资流水口径汇总 |
...
| 公会长名称 | 公会长昵称 | 关联查询用户昵称 筛选 用户ID = 公会长ID | 公会表关联用户表 |
| 所属BD名称 | 当前绑定BD昵称 | 关联查询用户昵称 筛选 BD编码 = 绑定BD的ID 且 当前时间 在 绑定生效时间 且 绑定失效时间 | 公会绑定关系表关联BD人员表关联用户表 |
| 所属BD ID | 当前绑定BD唯一标识 | 直接读取绑定BD的编码 筛选 当前时间 在 绑定生效时间 且 绑定失效时间 | 公会绑定关系表 |
| 所属BDM名称 | BD直属上级BDM昵称 | 关联查询上级BDM的用户昵称 筛选 上级BDM的ID = (从BD关系表查询BDM ID，筛选条件为BD ID等于绑定BD的ID) | BD关系表关联用户表 |
| 所属BDM ID | BD直属上级BDM唯一标识 | 直接读取上级BDM的编码 筛选 BD ID = 绑定BD的ID | BD关系表 |
| bonus到期时间 | 公会Bonus奖励截止日期 | 读取Bonus到期时间；当前时间在绑定有效期内；到期时间按首个绑定BD当月1号00:00:00 + 政策Bonus有效时长（自然月）计算，若首次绑定时间在16号00:00:00至月底期间，则额外+15个自然日 | 公会绑定关系表，格式YYYY-MM-DD HH:mm:ss |
| BD底薪到期时间 | 公会BD底薪奖励截止日期 | 读取底薪到期时间；当前时间在绑定有效期内；到期时间按首个绑定BD当月1号00:00:00 + 政策底薪有效时长（自然月）计算，若首次绑定时间在16号00:00:00至月底期间，则额外+15个自然日 | 公会绑定关系表，格式YYYY-MM-DD HH:mm:ss |
| 绑定时间 | 公会与BD建立绑定关系的时间 | 直接读取绑定日期 | 公会绑定关系表，格式YYYY-MM-DD HH:mm:ss |
| 操作时间 | 最近操作时间 | 直接读取更新时间 | 公会绑定关系表，格式YYYY-MM-DD HH:mm:ss |
| 操作人 | 最近操作人姓名 | 关联查询用户昵称 筛选 用户ID = 操作人ID | 公会绑定关系表关联用户表 |
| 操作 | 编辑 | 前端功能入口 |-|

#### 交互逻辑

**筛选查询：**
1. 点击"查询"按钮 → 根据筛选条件刷新列表。
2. 点击"重置"按钮 → 清空所有筛选条件，恢复默认状态。
3. 支持展开更多筛选条件。

**导出：**
1. 点击"导出excel"按钮 → 根据当前筛选条件导出绑定关系列表。
2. 导出范围：当前筛选条件下的全部数据，不分页。
3. 导出字段：公会名称、公会长ID、公会长名称、所属BD名称、所属BD ID、所属BDM名称、所属BDM ID、bonus到期时间、BD底薪到期时间、绑定时间、操作时间、操作人。
4. 导出格式：Excel（.xlsx），文件名格式：BD绑定公会详情_导出时间.xlsx。
5. 导出限制：单次导出最大10000条；超过10000条时提示"数据量过大，请缩小筛选范围后重试"。

**新增绑定：**
1. 点击"+ 新建绑定"按钮 → 打开新建绑定弹窗（11.2）。

**编辑绑定：**
1. 点击列表行"编辑"链接 → 打开编辑绑定弹窗（11.3），携带当前公会ID和绑定BD ID。

**分页：**
1. 默认每页10条，支持切换每页条数（10/20/50/100）。
2. 支持输入页码跳转。
...
新增一条BD与公会的绑定关系。输入公会ID后自动回填公会信息，填写目标BD ID后保存。

#### 字段与数据来源

| 字段 | 说明 | 控件类型 | 校验规则 | 数据来源 |
|---|---|---|---|---|
| 公会ID | 需要绑定的公会唯一标识 | 输入框 | 必填，仅可输入数字；失焦时查询公会信息并回填公会名称、公会长名称、公会长ID | 用户输入 |
| 公会名称 | 公会显示名称 | 只读文本 | - | 根据公会ID关联查询公会表 |
| 公会长名称 | 公会长昵称 | 只读文本 | - | 根据公会ID关联查询用户表 |
| 公会长ID | 公会长wechill账号ID | 只读文本 | - | 根据公会ID关联查询公会表 |

**下方输入：**

| 字段 | 说明 | 控件类型 | 校验规则 | 数据来源 |
|---|---|---|---|---|
| 所属BD ID | 需要绑定的BD唯一标识 | 输入框 | 必填，仅可输入数字；需校验BD是否存在且状态不为离职 | 用户输入 |

#### 交互逻辑

1. 打开弹窗时，公会ID、所属BD ID输入框为空，公会信息区隐藏。
2. 输入公会ID并失焦 → 校验公会是否存在：
   - 公会不存在：提示"公会不存在，请检查公会ID"
   - 公会已存在：回填公会名称、公会长名称、公会长ID，显示公会信息区
3. 输入所属BD ID → 失焦时校验BD是否存在且状态不为离职：
   - BD不存在：提示"BD不存在，请检查BD ID"
   - BD已离职：提示"该BD已离职，无法绑定"
4. 点击"取消" → 关闭弹窗，不保存。
5. 点击"绑定" → 校验通过后创建绑定关系：
   - 记录公会ID、绑定BD ID、绑定时间、操作人、bonus到期时间、BD底薪到期时间
   - bonus到期时间和BD底薪到期时间按政策有效时长计算：首次绑定时按绑定当月1号00:00:00 + 政策配置的Bonus有效时长（自然月）/底薪有效时长（自然月）；若绑定时间在16号00:00:00至月底期间，则额外+15个自然日
   - 创建成功后关闭弹窗，列表刷新，轻提示"绑定成功"
...
修改已有绑定关系的所属BD（换绑）。编辑场景下公会ID不可修改，仅可修改所属BD ID。

#### 字段与数据来源

| 字段 | 说明 | 控件类型 | 状态 | 数据来源 |
|---|---|---|---|---|
| 公会ID | 当前公会唯一标识 | 输入框 | 只读（灰底禁用） | 页面参数传入 |
| 公会名称 | 公会显示名称 | 只读文本 | - | 根据公会ID关联查询公会表 |
| 公会长名称 | 公会长昵称 | 只读文本 | - | 根据公会ID关联查询用户表 |
| 公会长ID | 公会长wechill账号ID | 只读文本 | - | 根据公会ID关联查询公会表 |
| 所属BD ID | 当前绑定BD ID | 输入框 | 可编辑 | 原绑定关系读取，可修改 |

#### 交互逻辑

1. 打开弹窗时，自动回填公会ID（只读）、公会名称、公会长名称、公会长ID、当前所属BD ID。
2. 所属BD ID可修改，失焦时校验BD是否存在且状态不为离职：
   - BD不存在：提示"BD不存在，请检查BD ID"
   - BD已离职：提示"该BD已离职，无法绑定"
3. 点击"取消" → 关闭弹窗，不保存。
4. 点击"绑定" → 校验通过后保存修改：
   - 更新公会绑定关系表的绑定BD ID、操作人、操作时间
   - **重要规则：换绑公会当月不按换绑时点切割数据**。未结算的当前自然月内，该公会数据、底薪有效薪资流水、Bonus有效薪资流水、BD提成、BDM团队统计均归当前新绑定BD及其当前所属BDM查看和结算。原绑定BD及其原所属BDM不能继续查看该公会当前月未结算数据，只能查看换绑前已生成的历史结算快照。
   - bonus到期时间和BD底薪到期时间**不因换绑重置**，仍按首个绑定BD当月1号00:00:00起算，沿用原到期时间。
5. 保存成功后关闭弹窗，列表刷新，轻提示"保存成功"。
---
...
- 换绑到新BD后，若发生在当前统计周期内，则可计入新BD的“本月新增公会数量”和“本月新增有效公会数量”。
...
| 当月新增公会数量 | 筛选范围内本月新增公会总数 | 统计筛选日期范围内绑定到筛选范围内BD的公会；不要求公会在筛选周期内首次创建，也不要求当前BD为首个绑定BD；同一公会在同一统计周期内对同一BD仅计一次 | 公会表 + 公会绑定关系表 |
...
| 当月底薪 | 筛选范围内所有BD的底薪汇总 | 汇总已生成结算记录中的底薪（base_salary）字段；该字段仅汇总底薪本身，不含提成、奖励、扣款 | 薪资结算表的底薪（base_salary）字段汇总 |
...
| 当月新增公会数量 | BD本月绑定到当前BD的公会数 | 统计筛选日期范围内绑定BD=当前BD的公会；不要求公会在筛选周期内首次创建，也不要求当前BD为首个绑定BD；同一公会在同一统计周期内对同一BD仅计一次 | 公会表 + 公会绑定关系表 |
...
| 本月底薪 | BD本月底薪 | 直接读取当前BD结算记录中的底薪（base_salary）字段；该字段表示按底薪有效薪资流水匹配当前等级后得到的现金底薪，不含提成、奖励、扣款 | 薪资结算表的底薪（base_salary）字段 |
| 金币奖励 | BD本月金币奖励数量 | 按BD等级配置和Bonus有效期计算 | 薪资结算表/奖励快照中的金币奖励字段 |
| 游戏币奖励 | BD本月游戏币奖励数量 | 按BD等级配置和Bonus有效期计算 | 薪资结算表/奖励快照中的游戏币奖励字段 |
| 当月新增公会未达标扣除金额 | BD本月未达标扣款 | 若新增有效公会数 < 目标，则(底薪+提成) × 扣款比例，否则为0 | 薪资结算表的扣款金额（deduct_amount）字段 |
...
- 当月新增公会数量：统计筛选日期范围内绑定到当前统计对象的公会数；不要求公会首次创建，也不要求当前BD为首个绑定BD。新增公会未达标扣款判断不直接使用该字段，统一改用“新增有效公会”字段。
...
| 当月新增公会数量 | 筛选范围内本月新增公会总数 | 统计筛选日期范围内绑定到BDM名下BD范围内的公会；不要求公会在筛选周期内首次创建，也不要求当前BD为首个绑定BD；同一公会在同一统计周期内对同一BD仅计一次 | 公会表 + 公会绑定关系表 + BD关系表 |
...
| 当月底薪 | 筛选范围内所有BDM的底薪汇总 | 汇总已生成结算记录中的底薪（base_salary）字段，筛选 BD角色（bd_role） = 1；该字段仅汇总底薪本身，不含提成、奖励 | 薪资结算表的底薪（base_salary）字段汇总 |
...
| 当月新增公会数量 | BDM团队本月绑定到团队BD的公会数 | 统计筛选日期范围内绑定到当前BDM团队BD范围内的公会；不要求公会在筛选周期内首次创建，也不要求当前BD为首个绑定BD；同一公会在同一统计周期内对同一BD仅计一次 | 公会表 + 公会绑定关系表 + BD关系表 |
...
| 本月底薪 | BDM本月底薪 | 直接读取当前BDM结算记录中的底薪（base_salary）字段；该字段表示按团队底薪有效薪资流水匹配当前等级后得到的现金底薪，不含提成、奖励 | 薪资结算表的底薪（base_salary）字段 |
...
- 当月：默认当前自然月；选择日期时，以选择范围为准
- 当月新增公会数量：统计筛选日期范围内绑定到当前统计对象的公会数；不要求公会首次创建，也不要求当前BD为首个绑定BD
...
- 停用政策时必须先校验该政策是否仍被在职BD使用；若存在在职BD正在使用，则禁止停用并提示“该政策已有在职BD使用中，无法停用。”
...
- 现金提现申请，但财务审核通过前必须复检余额，复检通过后先扣余额再打款。
