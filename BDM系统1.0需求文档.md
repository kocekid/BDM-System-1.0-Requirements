...
| 本月新增公会数量 | 统计本月绑定到当前BD的公会 | 统计周期内只要公会绑定到当前BD即计入本月新增公会数量；不要求首次创建，也不要求首绑；同一公会在同一统计周期内对同一BD仅计一次 |
...
- 新增公会定义：当前统计周期内绑定到当前BD的公会，就计入“新增公会”；不要求首次创建，也不要求首绑。新增有效公会则是在上述新增公会基础上，进一步满足Bonus有效薪资流水达到全局门槛。
...
- 换绑到新BD后，若发生在当前统计周期内，则可计入新BD的“本月新增公会数量”与“本月新增有效公会数量”；新增口径不再要求首次创建，也不再要求首绑。
...
| 本月新增公会数量 | 整数 | 统计本月绑定到当前BD的公会数量 | 公会服务：筛选统计周期内绑定BD为当前BD的公会；不要求首次创建，也不要求首绑；同一公会在同一统计周期内对同一BD仅计一次；该字段仅用于展示新增公会规模，不直接用于新增公会未达标扣款判断 |
...
| 本月新增有效公会数量 | 整数 | 统计本月绑定到当前BD、且Bonus有效薪资流水达到全局门槛的公会 | 公会表+公会绑定关系表，筛选统计周期内绑定BD为当前BD且Bonus有效薪资流水达到全局门槛的公会；不要求首次创建，也不要求首绑；同一公会在同一统计周期内对同一BD仅计一次；BD新增公会未达标扣款统一以该字段作为判断口径 |
...
| 月度新增有效公会目标 | BD角色专属，每月新增有效公会数目目标 | 直接输入写入新增有效公会目标；新增有效公会定义为统计周期内绑定到当前BD且Bonus有效薪资流水达到全局门槛的公会，不要求首次创建，也不要求首绑 | 用户输入，仅角色（role）=0时显示 |
...
| 月度新增有效公会目标 | BD角色专属，每月新增有效公会数目目标（仅BD政策有此字段） | 直接读取新增有效公会目标；新增有效公会定义为统计周期内绑定到当前BD且Bonus有效薪资流水达到全局门槛的公会，不要求首次创建，也不要求首绑 | 政策配置表或等级配置扩展字段，仅政策对象（policy_target）=BD时生效 |
...
| 月度新增有效公会目标 | BD角色专属，每月需达成的新增有效公会数目目标 | 直接输入整数；新增有效公会定义为统计周期内绑定到当前BD且Bonus有效薪资流水达到全局门槛的公会，不要求首次创建，也不要求首绑 | 用户输入；政策对象为BD时必填，BDM时不可编辑或隐藏 |
...
| 当月新增公会数量 | 筛选范围内本月新增公会总数 | 统计筛选日期范围内绑定到筛选范围内BD的公会；不要求首次创建，也不要求首绑；同一公会在同一统计周期内对同一BD仅计一次 | 公会表 + 公会绑定关系表 |
...
| 当月新增公会数量 | BD本月绑定到当前BD的公会数 | 统计筛选日期范围内绑定BD=当前BD的公会；不要求首次创建，也不要求首绑；同一公会在同一统计周期内对同一BD仅计一次 | 公会表 + 公会绑定关系表 |
...
- 当月新增公会数量：统计筛选日期范围内绑定到当前统计对象的公会数；不要求首次创建，也不要求首绑。新增公会未达标扣款判断不直接使用该字段，统一改用“新增有效公会”字段。
...
| 当月新增公会数量 | 筛选范围内本月新增公会总数 | 统计筛选日期范围内绑定到BDM名下BD范围内的公会；不要求首次创建，也不要求首绑；同一公会在同一统计周期内对同一BD仅计一次 | 公会表 + 公会绑定关系表 + BD关系表 |
...
| 当月新增公会数量 | BDM团队本月绑定到团队BD的公会数 | 统计筛选日期范围内绑定到当前BDM团队BD范围内的公会；不要求首次创建，也不要求首绑；同一公会在同一统计周期内对同一BD仅计一次 | 公会表 + 公会绑定关系表 + BD关系表 |
...
- 当月：默认当前自然月；选择日期时，以选择范围为准
- 当月新增公会数量：统计筛选日期范围内绑定到当前统计对象的公会数；不要求首次创建，也不要求首绑
